...
// Composerでquote可能なvaultアドレスリスト（on-chain確認済み）
const KNOWN_VAULTS = [
  { address: '0x4e65fe4dba92790696d040ac24aa414708f5c0ab', protocol: 'aave-v3',   name: 'USDC Vault', chainId: 8453, decimals: 6, symbol: 'aBasUSDC' },
  { address: '0x7BfA7C4f149E7415b73bdeDfe609237e29CBF34A', protocol: 'morpho-v1', name: 'Morpho USDC', chainId: 8453, decimals: 18, symbol: 'mUSDC' },
];
...
async function scanPositions(walletAddress) {
  ...
  for (const v of KNOWN_VAULTS) {
    try {
      ...
      if (bal === 0n) continue;
      const amount = parseFloat(ethers.formatUnits(bal, v.decimals));
      if (amount < 0.0001) continue;
      console.log(`  ✅ ${v.protocol} ${v.name}: ${amount.toFixed(4)} ${v.symbol}`);
      results.push({
        chainId: v.chainId,
...
async function withdrawPosition(position, walletAddress) {
  ...
  try {
    ...
    console.log(`\n📋 Withdrawal Plan:`);
    console.log(`  From  : ${position.amount.toFixed(4)} ${position.lpSymbol} (${position.protocol})`);
    ...
    const aavePool = AAVE_POOLS[position.chainId];
    if (aavePool && position.protocol === 'aave-v3') {
      // Aave直接withdraw（MaxUint256で利息込み全額）
      console.log('\n🏦 Withdrawing from Aave directly...');
      const pool = new ethers.Contract(aavePool, POOL_ABI, signer);
      const withdrawAmount = choice === '1' ? ethers.MaxUint256 : amountWei;
      const tx = await pool.withdraw(USDC[position.chainId], withdrawAmount, await signer.getAddress());
